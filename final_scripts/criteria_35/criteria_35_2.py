...
def get_component_pads_info():
    ...
    # 创建新的 DataFrame
    get_component_pads_info_re = pd.DataFrame({
        'original_row': filtered_df['_original_index'] + 2,
        'component_id': filtered_df['REFDES'].astype(str),
        'pad_id': filtered_df['REFDES'].astype(str) + '_' + filtered_df['PIN_NUMBER'].astype(str),
    ...
    get_component_pads_info_re.set_index('original_row', inplace=True)
    get_component_pads_info_re.sort_index(inplace=True)
    get_component_pads_info_re.index.name = None

    return get_component_pads_info_re
...
if __name__ == '__main__':
    ...
    result_df = get_component_pads_info()
    title = "🔍 criteria_35_2 - 获取元器件焊盘清单"
